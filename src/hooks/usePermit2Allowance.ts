...
export default function usePermit2Allowance(amount?: CurrencyAmount<Token>, spender?: string): Allowance {
  ...
  const { tokenAllowance, isSyncing: isApprovalSyncing } = useTokenAllowance(token, account, PERMIT2_ADDRESS(chainId))
  const updateTokenAllowance = useUpdateTokenAllowance(amount, PERMIT2_ADDRESS(chainId))
  ...
  const isApprovalPending = useHasPendingApproval(token, PERMIT2_ADDRESS(chainId))
  useEffect(() => {
