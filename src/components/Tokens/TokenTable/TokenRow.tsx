...
const StyledTokenRow = styled.div<{
  ...
  display: grid;
  font-size: 16px;
  grid-template-columns: 5.5fr 4fr 4fr 4fr 3.5fr;
  line-height: 24px;
  ...
  @media only screen and (max-width: ${MAX_WIDTH_MEDIA_BREAKPOINT}) {
    grid-template-columns: 6fr 4.5fr 4.5fr 4.5fr 4fr;
  }
...
export const HEADER_DESCRIPTIONS: Record<TokenSortMethod, ReactNode | undefined> = {
  ...
  [TokenSortMethod.TOTAL_VALUE_LOCKED]: (
    <Trans>
      Total value locked (TVL) is the aggregate amount of the asset available across all ISwap v3 liquidity pools.
    </Trans>
  ),
  // [TokenSortMethod.VOLUME]: (
  //   <Trans>Volume is the amount of the asset that has been traded on ISwap v3 during the selected time frame.</Trans>
  // ),
  ...
  return (
    <div ref={ref} data-testid={`token-table-row-${token.symbol}`}>
      <TokenRow
        ...
        operate={
          <Button padding='8px 16px' style={{ borderRadius: '6px', background: 'transparent' }} gap='8px' as={Link} to="/add/ETH"><Trans>Add Liquidity</Trans></Button>
        }
      />
    </div>
  )
